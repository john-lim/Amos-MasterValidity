...
#Region "Calculate CR-AVE-MSV-MR"
        ...
        For Each sRow As String In aRow
            ...
            If iCorrelations <> 0 Then
                ...
                For j = 1 To iCorrelations
                    If sRow = MatrixName(tableCorrelation, j, 2) Then
                        columnIndex = -1
                        rowIndex = -1
                        sColumn = MatrixName(tableCorrelation, j, 0)
                        dValue = MatrixElement(tableCorrelation, j, 3)
        ...
        For y = 0 To aRow.Length - 1 'Loops through all the rows in the matrix
            ...
            For x = 0 To aColumn.Length - 1 'Loops through all the columns in the matrix
                ...
                If x > 3 And aColumn(x) <> aRow(y) Then
                    For j = 1 To iCorrelations
                        If MatrixName(tableCovariance, j, 0) = aColumn(x) And MatrixName(tableCovariance, j, 2) = aRow(y) Then
                            If MatrixName(tableCovariance, j, 6) = "***" Then
                                significance = "***"
                            ElseIf MatrixElement(tableCovariance, j, 6) < 0.01 Then
                                significance = "**"
                            ElseIf MatrixElement(tableCovariance, j, 6) < 0.05 Then
                                significance = "*"
                            ElseIf MatrixElement(tableCovariance, j, 6) < 0.1 Then
                                significance = "&#8224;"
                            Else
                                significance = ""
                            End If
